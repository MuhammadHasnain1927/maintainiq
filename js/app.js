...
const CATEGORIES = ["Electrical","HVAC","Plumbing","IT & AV","Furniture","Machinery","Safety","Cleaning","Structural","Other"];
const CONDITIONS = ["Good","Fair","Poor"];

const ISSUE_FLOW = {
  "Reported":["Assigned"],
  "Assigned":["Inspection Started"],
  "Inspection Started":["Maintenance In Progress","Waiting for Parts","Resolved"],
...
function chipClass(v){ return (v||"").toLowerCase().replace(/\s+/g,"-"); }
...
async function getProfile(){
  ...
  const p=data ? {...data, email:session.user.email} : {id:session.user.id, full_name:session.user.email, role:"technician", email:session.user.email};
...
function refreshProfileInBackground(){
  getProfile().then(p=>{
    if(p && p.active===false){
      ...
      db.auth.signOut().then(()=>location.href="index.html?deactivated=1");
    }
  }).catch(()=>{});
}
...
async function requireAuth(){
  const session=await getSession();
  if(!session){ sessionStorage.removeItem("miq_profile"); location.href="index.html"; return null; }
  const cached=sessionStorage.getItem("miq_profile");
  if(cached){
    try{
      const p=JSON.parse(cached);
      if(p.id===session.user.id){ refreshProfileInBackground(); return p; }
    }catch(e){}
  }
...
function confirmDialog(title,msg,okLabel="Confirm",danger=false){
  return new Promise(resolve=>{
    ...
    document.addEventListener("keydown",function esc_(e){ if(e.key==="Escape"){ document.removeEventListener("keydown",esc_); done(false); } });
  });
}

function exportCSV(rows,columns,filename){
  if(!rows.length){ toast("Nothing to export.",true); return; }
  const escape=v=>{ v=v==null?"":String(v); return /[",\n]/.test(v)?`"${v.replace(/"/g,'""')}"`:v; };
  const csv=[columns.map(c=>escape(c.label)).join(","),
    ...rows.map(r=>columns.map(c=>escape(typeof c.get==="function"?c.get(r):r[c.get])).join(","))].join("\n");
...
async function logHistory(assetId, action, actor, issueId=null){
  await db.from("asset_history").insert({asset_id:assetId, action, actor:actor||"System", issue_id:issueId});
}
...
const ICONS={
  ...
  asset:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 8V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v2"/><rect x="3" y="8" width="18" height="12" rx="2"/><path d="M9 12h6"/></svg>',
  issue:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>',
  ...
  spark:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v3m0 12v3M5.6 5.6l2.1 2.1m8.6 8.6 2.1 2.1M3 12h3m12 0h3M5.6 18.4l2.1-2.1m8.6-8.6 2.1-2.1"/><circle cx="12" cy="12" r="3.2"/></svg>'
};
...
function renderShell(active, profile){
  ...
  const nav=[
    {id:"dashboard",href:"dashboard.html",label:"Dashboard",icon:ICONS.dash},
    {id:"assets",href:"assets.html",label:"Assets",icon:ICONS.asset},
    {id:"issues",href:"issues.html",label:"Issues",icon:ICONS.issue}
  ];
  if(isAdmin) nav.push({id:"team",href:"team.html",label:"Team",icon:ICONS.team});
  const side=document.getElementById("sidebar");
  side.innerHTML=`
    <div class="brand"><div class="brand-mark">M</div><div class="brand-name">Maintain<span>IQ</span></div></div>
  ...
  document.getElementById("profileBtn").onclick=()=>{
    ...
    document.getElementById("profileBtn").setAttribute("aria-expanded",menu.classList.contains("open"));
  };
  ...
  if(ham){
    ...
    document.addEventListener("click",e=>{
      if(side.classList.contains("open") && !side.contains(e.target) && e.target!==ham && !ham.contains(e.target)) side.classList.remove("open");
    });
  }
  return isAdmin;
}
